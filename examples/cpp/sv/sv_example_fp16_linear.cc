...
/// all feats are fp16, and format are kLINEAR
...
void test(size_t batch_size, size_t seq_len, size_t embed_dim, size_t head_num, size_t layer_num)
{
    ...
    data_out_path = "sv_example_fp16_linear_nc" + std::to_string(num_cams) + "/";
    ...
    half* lidar2img = nullptr;
    float* lidar2img_f = nullptr;
    if (num_cams == 4) {
        deviceMalloc(&lidar2img, batch_size * num_cams * 4 * 4, false);
    }
    else {
        deviceMalloc(&lidar2img_f, batch_size * num_cams * 4 * 4, false);
    }
    ...
    for (int idx = 1; idx < 12; ++idx) {
        ...
        std::vector<Tensor> input_tensors{Tensor{MEMORY_GPU,
                                                 getTensorType<T>(),
        ...
        if (num_cams == 4) {
            ...
            input_tensors.emplace_back(
                Tensor{MEMORY_GPU, getTensorType<half>(), std::vector<size_t>{num_cams, 4, 4}, lidar2img});
        }
        else {
            cudaacc::Read2Dptr(inpp + cnn_feats_names_str[3], {int(batch_size), int(num_cams), 4, 4}, lidar2img_f);
            input_tensors.emplace_back(
                Tensor{MEMORY_GPU, getTensorType<float>(), std::vector<size_t>{num_cams, 4, 4}, lidar2img_f});
        }
        ...
        cudaacc::WriteFromDptr(
            outpp + "Out.outputs_bbox.1-512-8", {int(batch_size), int(seq_len), int(num_reg_points)}, last_reg_out);
        cudaacc::WriteFromDptr(
            outpp + "Out.outputs_scores.1-512-8", {int(batch_size), int(seq_len), int(num_classes)}, last_cls_out);
    ...
    if (lidar2img != nullptr)
        check_cuda_error(cudaFree(lidar2img));
    if (lidar2img_f != nullptr)
        check_cuda_error(cudaFree(lidar2img_f));
...
int main(int argc, char* argv[])
{
    ...
    test<half>(batch_size, seq_len, embed_dim, head_num, layer_num);
