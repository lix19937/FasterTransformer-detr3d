...
void ViTTransformer<T>::initialize()
{
    ...
    if ((attention_type_ == AttentionType::FUSED_MHA) && std::is_same<T, half>::value == true && max_seq_len_ <= 384) {
        ...
        if (request_seq_len_ % 8 != 0 && std::is_same<half, T>::value) {
            max_seq_len_ = (request_seq_len_ + 7) / 8 * 8; /// 900 --> 904
                                ...
                                const ViTWeight<T>* weights)
{
    ...
    const bool need_padding =
      ...
      //FT_LOG_INFO("forward seq_len:%d  need_padding:%d", seq_len, need_padding?1:0);
    ...
    T* from_buf = encoder_input_ptr;//// input_batch_size * seq_len * embed_dim_   1, L, 256
    T* norm_out_buf = embed_buf_2_; //// input_batch_size * seq_len * embed_dim_   1, L, 256
    T* attn_out_buf = embed_buf_3_; //// input_batch_size * seq_len * embed_dim_   1, L, 256
    T* encoder_out_buf = from_buf;

    for (uint i = 0; i < num_layer_; i++) {
        FT_LOG_INFO("forward invokeGeneralLayerNorm     %d ", i);

        // LN 
        invokeGeneralLayerNorm(norm_out_buf, /// OUT
                               from_buf, /// IN 
        ...
        FT_LOG_INFO("forward attention_layer_     %d  h_token_num:%d  %d", i, h_token_num, seq_len); // 904  904  exit(0);                    
        // self-Attention
        {

            std::vector<Tensor> attn_input_tensors{
                Tensor{MEMORY_GPU, data_type, std::vector<size_t>{h_token_num, embed_dim_}, norm_out_buf},
                Tensor{MEMORY_GPU, data_type, std::vector<size_t>{input_batch_size, 1, seq_len, seq_len}, 
                  mask_buf_},
                *offset_tensor_ptr};
        ...
        FT_LOG_INFO("forward invokeGeneralAddBiasResidualPreLayerNorm     %d ", i);                       
        //  + bias + residual + ln
        invokeGeneralAddBiasResidualPreLayerNorm(
            from_buf, /// out
            norm_out_buf,//// add this 
            attn_out_buf,//// from mha`s out
            weights->vit_layer_weights[i].ffn_layernorm_weights.gamma,//// ln`s g
            weights->vit_layer_weights[i].ffn_layernorm_weights.beta,//// ln`s b
            weights->vit_layer_weights[i].attention_weights.attention_output_weight.bias, ///
            h_token_num,
            embed_dim_,
            stream_);

        FT_LOG_INFO("forward ffn_layer_     %d ", i);                       
        // FFN ,  svod use relu not gelu
        ///
        {
        ...
        FT_LOG_INFO("forward invokeAddBiasResidual     %d ", i);  
        ///   from_buf +=  attn_out_buf + bias                   
    ...
    FT_LOG_INFO("forward invokeGeneralLayerNorm      h_token_num:%d, embed_dim_:%d", h_token_num, embed_dim_);                       
    ...
    if (need_padding) {
              //FT_LOG_INFO("forward invokeRemovePadding     ");                       
