 ...
 * @Last Modified time: 2022-08-14 18:57:07
 **************************************************************/
...
#include "helper_string.h"  // NOLINT
...
static const char *_cudaGetErrorEnum(cusolverStatus_t error) {
  switch (error) {
    ...
    case CUSOLVER_STATUS_INVALID_WORKSPACE:
      return "CUSOLVER_STATUS_INVALID_WORKSP";
#endif
    default:
      break;
  }
...
template <typename T>
void check_v2(T result, char const *const func, const char *const file,
             int const line) {
  if (result) {
    struct timeval tv;
    ...
    fprintf(
        stderr,
        "\033[31m%s.%06d: %s %s:%d] CUDA error code=%d(%s) \"%s\" \033[37m\n",
        time_buffer, micros_remainder, "E", file, line,
        static_cast<unsigned int>(result), _cudaGetErrorEnum(result), func);
    // DEVICE_RESET
    // Make sure we call CUDA Device Reset before exiting
...
#define CheckCudaErrors(val) check_v2((val), #val, __FILE__, __LINE__)
