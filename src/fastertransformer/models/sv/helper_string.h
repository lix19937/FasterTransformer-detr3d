...
template <>
inline uint8_t exchange(const std::string &src) {
  return atoi(src.c_str());
}
...
void Read2Buff(const std::string &file_name,
  ...
  /// int
  h_data.resize(len);
  if (!cudaacc::readbyw(file_name, h_data.data(), c, h, w)) {
    printf("readrp error %s, chw:%d %d %d\n", file_name.c_str(), c, h, w);
    exit(1);
  }
}
...
void Read2Buff(const std::string &file_name,
  ...
  /// int
  if (!cudaacc::readbyw(file_name, h_data, c, h, w)) {
    printf("readrp error %s, chw:%d %d %d\n", file_name.c_str(), c, h, w);
    exit(1);
  }
}
