...
int main(int argc, char* argv[])
{
    if (argc != 8) {
        ...
        printf("e.g. ./build/bin/gsd_gemm 1 332 960 4 1 1 0 \n");// fp32
        return 0;
    }
    ...
    size_t buf_size_in_byte =
        ft::GSDcalGemmTestBufSizeInByte(batch_size, seq_len, head_num, size_per_head, 0, 0, int8_mode, data_type);
    size_t total, free;
    ...
    printf("buf_size_in_byte:%zd\n", buf_size_in_byte);//exit(0);
    ...
    if (int8_mode != 0) { 
      /// enter int8
       // ft::generate_encoder_igemm_config(batch_size, seq_len, head_num, size_per_head, gemm_test_buf, false);
        ...
        ft::generate_gsd_gemm_config<float>(batch_size, seq_len, head_num, size_per_head, gemm_test_buf, false);
    }
    else if (data_type == ft::HALF_DATATYPE) {
        ft::generate_gsd_gemm_config<half>(batch_size, seq_len, head_num, size_per_head, gemm_test_buf, false);
    }
    else {
