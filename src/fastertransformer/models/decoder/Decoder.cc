...
const char *__mem_type_map[]{
    "MEMORY_CPU",
    "MEMORY_CPU_PINNED",
    "MEMORY_GPU"
};

void print_tensor(const Tensor &t, int idx){
  printf("\n %d, %s, %s, ", idx, __mem_type_map[int(t.where)], __type_map[int(t.type)]);
  for (auto it: t.shape) printf("%ld ", it);
  printf(",data:%p\n", t.data);
  if (int(t.where) == 0 && t.shape.size()==1)  printf(", %d\n", *(int *)(t.data));
  // if (int(t.where) == 2 && int(t.type)==8 && (t.shape.size()==1 
  //   || (t.shape.size()==2 && t.shape[0] == t.shape[1] && t.shape[0] == 1))){
  //   int *h_t = (int *)malloc(4);
  //   cudaMemcpy(h_t, t.data, 4, cudaMemcpyDeviceToHost);
  //   printf(", %d\n", *h_t);
  //   free(h_t);
  // }
                         ...
                         const std::vector<DecoderLayerWeight<T>>* decoder_layer_weight)
{
    ...
    for (uint l = 0; l < num_layer_; l++) {
        ...
        std::vector<Tensor> self_attention_input_tensors{
            Tensor{MEMORY_GPU, data_type, {batch_size, hidden_units_}, decoder_normed_input_},//#0
            input_tensors->at(3),//#1 finished
            input_tensors->at(5),//#2 sequence_lengths
            Tensor{MEMORY_GPU, TYPE_INT32, {batch_size}, nullptr},//#3
            Tensor{MEMORY_CPU, TYPE_INT32, {1}, &tmp_0},//#4
            input_tensors->at(4),//#5
            input_tensors->at(6)};//#6
        ...
        for (size_t i=0; i<self_attention_input_tensors.size(); ++i)
          print_tensor(self_attention_input_tensors[i], i);
        ...
        for (size_t i=0; i<self_attention_output_tensors.size(); ++i)
          print_tensor(self_attention_output_tensors[i], i);
        printf(" ===== ma done =====\n");
        ///////////////////////////////////////////////////////////////////////////////////////////////////
