...
void ClsBranch(float* buf[],    // fc1_out,  fc2_out
               float* cls_out,  // block of ffn_ln`s output  [L, embed_dims]
               const float* in,
               const ClsBranchWeight<float>* weights,
               int m,   /* L */
               int k,   /* embed_dims */
               int ich, /* embed_dims */
               int och, /* 5 or oth num of class */
...
template<>
void SVClsLayer<float>::__forward(std::vector<fastertransformer::Tensor>* output_tensors,
                                  ...
                                  const ClsBranchWeight<float>* weights)
{
    ...
    /// IN
    auto d_in_ffn_ln_output = (const float*)input_tensors->at(0).data; /* [1, L, embed_dims]  add with query_pos */
