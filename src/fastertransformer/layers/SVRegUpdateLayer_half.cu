...
__global__ void IsigmoidAddBiasKernel(half* out,
                                      const half* __restrict__ reg, /* map to `tmp`, [L, 8] */
                                      const half* __restrict__ rp,  /* map to `reference_points` [L, 3] */
                                      ...
                                      const int k)
{
    ...
    // const half eps = 1.e-5;
    // auto x = __ldg(&rp[rp_idx]);
    // x = __hmin(__hmax(x, 0.f), 1.f);
    // float x1 = __hmax(x, eps);
    // float x2 = __hmax(__hsub(__float2half(1.f), x), eps);
    // float a = __hadd(__ldg(&reg[row_idx * k + reg_offset_map[col_idx]]), __ldg(&bias[reg_offset_map[col_idx]]));
    ...
    out[rp_idx] = 1.f / (1.f + __expf(-(log(x1 / x2) + a)));
}
...
__global__ void IsigmoidAddBiasKernelFused(float* out,                    /* [L, 8] */
                                           const half* __restrict__ in,   /* map to tmp but no bias */
                                           const half* __restrict__ rp,   /* map to reference_points */
                                           const half* __restrict__ bias, /* [1, 8] */
                                           ...
                                           const int k)
{
    ...
    if (col_idx > 4 || col_idx == 2 || col_idx == 3) {
        out[reg_idx] = __hadd(__ldg(&in[reg_idx]), __ldg(&bias[col_idx]));
        return;
    }
    ...
    float a = __hadd(__ldg(&in[reg_idx]), __ldg(&bias[col_idx]));
...
void RegBranch(half* buf[],            /* fc1_out,  fc2_out */
               const half* ffn_ln_out, /* block of ffn_ln`s output  [L, embed_dims] */
               const RegBranchWeight<half>* weights,
               int m,   /* L */
               int k,   /* embed_dims */
               int ich, /* embed_dims */
               int och, /* 8, hard code*/
...
void Update(half* out,           /* [L, 1, 3] */
            const half* reg,     /* from RegBranch but no bias [L, 1, 8] */
            const half* rp,      /* reference_points [L, 3] */
            const half* fc_bias, /* [1, 8] */
            const int m,         /* L */
            const int n,         /* 3 */
            const int k,         /* 8 */
            cudaStream_t stream)
{
    ...
    half16::IsigmoidAddBiasKernel<<<grid, block, 0, stream>>>(out, reg, rp, fc_bias, n, k);
}

void UpdateFused(float* out,            /* [L, 1, 8] */
                 const half* in,        /* from RegBranch but no bias [L, 1, 8] */
                 const half* rp,        /* reference_points [L, 3] */
                 const half* fc_bias,   /* [1, 8] */
                 ...
                 cudaStream_t stream)
{
    ...
    half16::IsigmoidAddBiasKernelFused<<<grid, block, 0, stream>>>(out, in, rp, fc_bias, pc_range, n, k);
}

template<>
void SVRegUpdateLayer<half>::__forward(std::vector<fastertransformer::Tensor>* output_tensors,
                                       const std::vector<fastertransformer::Tensor>* input_tensors,
                                       const RegBranchWeight<half>* weights)
{
    /// OUT
    auto d_rp_new = (half*)output_tensors->at(0).data; /* [L, 1, 3] */

    /// IN
    auto in_ffn_ln_output = (const half*)input_tensors->at(0).data;  // [L, embed_dims]  add with query_pos
    auto in_rp = (const half*)input_tensors->at(1).data;             // [1, L, 3] reference_points
...
template<>
void SVRegUpdateLayer<half>::forward_fused(std::vector<fastertransformer::Tensor>* output_tensors,
                                           const std::vector<fastertransformer::Tensor>* input_tensors,
                                           const RegBranchWeight<half>* weights,
                                           cudaStream_t stream)
{
    ...
    half* inner_fc_buf[]{fc_buf_[0], fc_buf_[1], fc_buf_[0]};
