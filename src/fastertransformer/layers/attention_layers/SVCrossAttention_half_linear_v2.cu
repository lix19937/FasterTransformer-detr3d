...
/// SVCrossAttention_half_linear.cu optimized version 2

namespace half_linear_v2 {
                                                               ...
                                                               const int inp_W)
{
    ...
    if (within_bounds_2d(iy_nw_i, ix_nw_i, inp_H, inp_W)) {
        find_maxpool2d_src(inp_ptr_NC + offset1, area, inp_W, input, &value);
        *sum = __hfma(value, nw, *sum);
    }

    if (within_bounds_2d(iy_nw_i, ix_nw_1_i, inp_H, inp_W)) {
        find_maxpool2d_src(inp_ptr_NC + offset2, area, inp_W, input, &value);
        *sum = __hfma(value, ne, *sum);
    }

    if (within_bounds_2d(iy_nw_1_i, ix_nw_i, inp_H, inp_W)) {
        find_maxpool2d_src(inp_ptr_NC + offset3, area, inp_W, input, &value);
        *sum = __hfma(value, sw, *sum);
    }

    if (within_bounds_2d(iy_nw_1_i, ix_nw_1_i, inp_H, inp_W)) {
        find_maxpool2d_src(inp_ptr_NC + offset4, area, inp_W, input, &value);
...
__global__ void grid_sampler_2d_forward_kernel_n6c256gh512gw1_chw32(
    ...
    const half* __restrict__ input3,
    ...
    const int reduce_num)
{
    ...
    __shared__ half2 shmm_x[96];  ////  lix19937
    __shared__ half2 shmm_y[96];
    ...
    get_interpolation(n * inp_sN, &sum12.x, input, grid, blockIdx.y, inp_sH, inp_sC, out_sC, grid_offset, inp_H, inp_W);

    get_interpolation(
        n * inp_sN2, &sum12.y, input2, grid, blockIdx.y, inp_sH2, inp_sC2, out_sC, grid_offset, inp_H2, inp_W2);

    get_interpolation(
        n * inp_sN3, &sum34.x, input3, grid, blockIdx.y, inp_sH3, inp_sC3, out_sC, grid_offset, inp_H3, inp_W3);

    get_interpolation_with_maxpool(
        n * inp_sN4, &sum34.y, input3, grid, blockIdx.y, inp_sH4, inp_sC4, out_sC, grid_offset, inp_H4, inp_W4);
    ...
    if (threadIdx.y == 0) {
        if (FEATURE_NC == 12) {
            shmm_x[_tid] = __hadd2(shmm_x[_tid], shmm_x[_tid + 5]);
            shmm_y[_tid] = __hadd2(shmm_y[_tid], shmm_y[_tid + 5]);
            shmm_x[_tid] = __hadd2(shmm_x[_tid], shmm_x[_tid + 4]);
            shmm_y[_tid] = __hadd2(shmm_y[_tid], shmm_y[_tid + 4]);
            shmm_x[_tid] = __hadd2(shmm_x[_tid], shmm_x[_tid + 3]);
            shmm_y[_tid] = __hadd2(shmm_y[_tid], shmm_y[_tid + 3]);
        }

        shmm_x[_tid] = __hadd2(shmm_x[_tid], shmm_x[_tid + 2]);
        shmm_y[_tid] = __hadd2(shmm_y[_tid], shmm_y[_tid + 2]);
    ...
    cudaStream_t stream)
{
    ...
    if (nc == 12) {
        ...
        grid_sampler_2d_forward_kernel_n6c256gh512gw1_chw32<6>
            <<<grid, block, 0, stream>>>(grid_H,
                                         mlvl_feats[0],
                                         mlvl_feats[1],
                                         mlvl_feats[2],
...
template<>
void SVCrossAttentionLayer<half>::__forward_half_linear_v2_branch(
    ...
    const CAttentionWeight<half>* weights)
{
    ...
    half_linear_v2::ReferencePointsNorm(d_rp_norm_,                /* rp after norm; [1, 4, num_query] */
    ...
    half_linear_v2::L2IDivImgShpe(d_l2i_norm_,  /* [NC, 4, 4] */
    ...
    half_linear_v2::L2IxReferencePoints((half*)d_rp_matmuled_,    /* rp after matmul; [1, NC, 4, L] */
    ...
    /// d_rp_matmuled_(half) --> d_fs_mask_(uint8) + d_rpc_norm_ (half)
    half_linear_v2::ReferencePointsCamAndMask(
        d_fs_mask_,  /* last out; [B, Ch, L, NC, 1, 1] */
    ...
    half_linear_v2::AttentionWeightsFc(d_attention_weights_output_, /* output; [L, 24] , no bias*/
    ...
    half_linear_v2::BatchGridsampleTwoMulReduceSum(
    ...
    half_linear_v2::OutputProjFc(d_output, /* last output;  [L, embed_dims] */
    ...
    half_linear_v2::PositionEncoder(d_pos_feat,       /* last output;  [L, embed_dims] */
...
template<>
void SVCrossAttentionLayer<half>::__forward_magic_half_linear_v2_branch(
    std::vector<fastertransformer::Tensor>* output_tensors,
    const std::vector<fastertransformer::Tensor>* input_tensors,
    const CAttentionWeight<half>* weights,
    ...
    cudaStream_t stream)
{
    ...
    half_linear_v2::ReferencePointsCamAndMask(d_fs_mask_,  /* last out; [B, Ch, L, NC, 1, 1] */
