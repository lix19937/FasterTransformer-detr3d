...
template<typename T>
void SVClsLayer<T>::forward(std::vector<fastertransformer::Tensor>* output_tensors,
...
template<typename T>
SVClsLayer<T>::SVClsLayer(size_t max_batch_size,
                                      size_t max_seq_len,
                                      size_t hidden_units,
                                      float q_scaling,
                                      cudaStream_t stream,
...
template<typename T>
SVClsLayer<T>::SVClsLayer(SVClsLayer<T> const& layer):
...
template<typename T>
void SVClsLayer<T>::allocateBuffer()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void SVClsLayer<T>::freeBuffer()
{
    allocator_->free(fc1_buf_);
    allocator_->free(fc2_buf_);
}

template class SVClsLayer<float>;
template class SVClsLayer<half>;
