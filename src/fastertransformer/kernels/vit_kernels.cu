...
////////////////////////////////////////////////////////////////////////////////////////////////
template<typename T>
__global__ void add_posembed(T* __restrict out,              // b*(h*w)*n
                             const T* __restrict in,         // (h*w)*n
                             const T* __restrict pos_embed,  // (h*w)*n
                             const int n)
{
    for (int id = blockIdx.x * blockDim.x + threadIdx.x; id < n; id += blockDim.x * gridDim.x) {
        out[id] = __ldg(&in[id]) + __ldg(&pos_embed[id]);
    }
}

template<>
__global__ void add_posembed(half* __restrict out,              // b*(h*w+1)*n
                             const half* __restrict in,         // (h*w)*n
                             const half* __restrict pos_embed,  // (h*w)*n
                             const int n)
{
    half2* out_ptr = (half2*)out;
    ...
    for (int id = blockIdx.x * blockDim.x + threadIdx.x; id < n; id += blockDim.x * gridDim.x) {
        half2 d1 = __ldg(&in_ptr[id]);
        half2 d2 = __ldg(&embed_ptr[id]);
        out_ptr[id] = __hadd2(d1, d2);
    }
}

template<typename T>
void invokeAddPosEmbed(T* out, const T* in, const T* pos_embed, const int m, const int n, cudaStream_t stream)
{
    const int data_type_factor = 4 / sizeof(T);  // 1 for fp32, 2 for fp16
    dim3 block, grid;
    auto th = (n >> 2) / data_type_factor;
    if (th <= 1024) {
        block.x = th;
        grid.x = m;
    }
    ...
    add_posembed<<<grid, block, 0, stream>>>(out, in, pos_embed, m * n / data_type_factor);
}

template void
invokeAddPosEmbed(float* out, const float* in, const float* pos_embed, const int m, const int n, cudaStream_t stream);

template void
invokeAddPosEmbed(half* out, const half* in, const half* pos_embed, const int m, const int n, cudaStream_t stream);
