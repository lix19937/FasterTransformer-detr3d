...
__global__ void generalAddBiasResidualLayerNormOpt(T* __restrict normed_output,
                                                   T* __restrict output,
                                                   const T* __restrict bias,
                                                   const T* __restrict residual,
                                                   const T* __restrict input_exp,
...
#pragma unroll
    for (int i = threadIdx.x; i < n; i += blockDim.x) {
        ...
        if (IS_RESIDUAL) {
            ...
            val = hadd2(val, ldg(&input_exp[index]));
        }
                                                ...
                                                int n)
{
    ...
    for (int i = tid; i < n; i += blockDim.x) {
        ...
        float local_out = (float)(ldg(&input[it])) + (float)(ldg(&input_exp[it]));
    ...
    for (int i = tid; i < n; i += blockDim.x) {
        ...
        norm_output[blockIdx.x * n + i] =
            (T)((((float)output[blockIdx.x * n + i] - s_mean) * s_variance) * (float)(ldg(&gamma[i]))
                + (float)(ldg(&beta[i])));
    }
}
                                              ...
                                              int opt_version)
{
    if (opt_version > 0 && sizeof(T) == 2 && (n % 2 == 0)) {  ///
        ...
        if (opt_version == 1) {
          ...
          //printf("  >>> opt_version %d  unroll_factor :%d \n\n", opt_version, unroll_factor);
        ...
        */
        ...
        generalAddBiasResidualLayerNorm<T>
            <<<grid, block, 0, stream>>>(input, input_exp, gamma, beta, bias, output, norm_output, m, n);  // For gpt-3
    }
}
