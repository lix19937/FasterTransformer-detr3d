...
///  use cuda kernel 
const bool use_native_api{true};
...
{
    printf("gemm %s%s {%d %d %d, %d %d %d} %d %d %d %d [line:%d]\n",
           int(transa) == 0 ? "N" : "T",
           int(transb) == 0 ? "N" : "T",
           m, n, k, lda, ldb, ldc,
    ...
    if (use_native_api) {
        ...
        const bool is_fp16 = Atype == cudaDataType_t::CUDA_R_16F;
        native::Gemm(transa, transb, m, n, k, A, lda, B, ldb, C, ldc, f_alpha, f_beta, is_fp16, stream_);
        return;
    }
...
void cublasMMWrapper::Gemm(cublasOperation_t transa,
                           ...
                           float f_beta)
{
    printf("gemm %s%s {%d %d %d, %d %d %d, %f %f} [line:%d]\n",
           int(transa) == 0 ? "N" : "T",
           int(transb) == 0 ? "N" : "T",
           m, n, k, 
           lda,        // m
    ...
    if (use_native_api) {
        const bool is_fp16 = Atype_ == cudaDataType_t::CUDA_R_16F;
        native::Gemm(transa, transb, m, n, k, A, lda, B, ldb, C, ldc, f_alpha, f_beta, is_fp16, stream_);
        return;
    }
    ...
    bool using_cublasLt = Atype_ == CUDA_R_16F;
...
void cublasMMWrapper::Gemm(cublasOperation_t transa,
                           ...
                           const int ldc)
{
    printf("gemm %s%s {%d %d %d, %d %d %d} [line:%d]\n",
           int(transa) == 0 ? "N" : "T",
           int(transb) == 0 ? "N" : "T",
           m, n, k, 
           lda,        // k
...
void cublasMMWrapper::stridedBatchedGemm(cublasOperation_t transa,
                                         ...
                                         const float f_beta)
{
    printf("stridedBatchedGemm  %s%s {%d %d %d, %d %d %d, %ld %ld %ld, %d} %f %f [line:%d]\n",
           int(transa) == 0 ? "N" : "T",
           int(transb) == 0 ? "N" : "T",
           m,n,k,
           lda,        
    ...
    if (use_native_api) {
        const bool is_fp16 = Atype_ == cudaDataType_t::CUDA_R_16F;
        native::stridedBatchedGemm(transa,
                                   transb,
                                   m,
                                   n,
                                   k,
...
void cublasMMWrapper::stridedBatchedGemm(cublasOperation_t transa,
                                         ...
                                         cudaDataType_t computeType)
{
    printf("stridedBatchedGemm ++++++++++++++++++++++++ no enter \n");   exit(0);
...
void cublasMMWrapper::batchedGemm(cublasOperation_t transa,
                                  ...
                                  const int batch_count)
{
    printf("batchedGemm not enter +++++++++++++++++ \n"); exit(0);

    if (use_native_api) {
        native::batchedGemm(transa, transb, m, n, k, A, lda, B, ldb, C, ldc, batch_count);
        return;
    }
                             ...
                             void* C)
{
    ...
    if (computeType_ != CUDA_R_16F && not_printed_fp32_accumulation_warning) {
        printf("[WRANING] cublasMMWrapper sets to FP32 compute type, "
