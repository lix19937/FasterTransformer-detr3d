...
cublasAlgoMap::cublasAlgoMap(const std::vector<std::string> &choices, const std::vector<std::string> spchoices){
    loadGemmConfig(choices);
    loadSpGemmConfig(spchoices);
}
...
void cublasAlgoMap::loadGemmConfig(const std::vector<std::string> &choices)
{
...
void cublasAlgoMap::loadGemmConfig()
{
    ...
    if (fd == NULL) {
        FT_LOG_INFO("[WARNING]: %s is not found; using default GEMM algo\n", config_filename_.c_str());
        return;
    }
...
cublasAlgoMap::getAlgo(const int batch_count, const int m, const int n, const int k, const CublasDataType data_type)
{
    char mark[256]{0};
    sprintf(mark, "%d_%d_%d_%d_%d", batch_count, n, m, k, data_type);
    //printf(" -->> algo_map_.size: %zd,  %s\n", algo_map_.size(), mark); // not use app_printf
    if (algo_map_.find(mark) != algo_map_.end()) {
        return algo_map_[mark];
    }
    else {
        // printf(">>not find algo_map_.size: %zd,  %s\n", algo_map_.size(), mark); /// lix add
...
void cublasAlgoMap::loadSpGemmConfig(const std::vector<std::string> &spchoices)
{
    if (spchoices.empty()) {
        return;
    }
