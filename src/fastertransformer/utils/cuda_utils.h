...
void check(T result, char const* const func, const char* const file, int const line)
{
    if (result) {
        ...
        throw std::runtime_error(std::string("[ERROR] CUDA runtime error: ") + (_cudaGetErrorEnum(result)) + " "
                                 + file_str_short + ":" + std::to_string(line) + " \n");
    }
}
...
inline void syncAndCheck(const char* const file, int const line)
{
    ...
    if (result) {
        ...
        auto pos = file_str.rfind("/");                                                                              
        auto file_str_short = file_str.substr(pos + 1); 
        throw std::runtime_error(std::string("[ERROR] [syncAndCheck] CUDA runtime error: ") + (_cudaGetErrorEnum(result)) + " "
                                 + file_str_short + ":" + std::to_string(line) + " \n");
    }
#endif
}
...
void print_abs_mean(const T* buf, uint size, cudaStream_t stream, std::string name = "")
{
    ...
    printf("[INFO] %20s size: %u, abs mean: %f, abs sum: %f, abs max: %f, find inf: %s",
           name.c_str(),
           size,
           sum / size,
           sum,
...
inline void myAssert(bool result, const char* const file, int const line, std::string info = "")
{
    if (result != true) {
        ...
        auto pos = file_str.rfind("/");                                                                              
        auto file_str_short = file_str.substr(pos + 1); 
        throw std::runtime_error(std::string("[ERROR] ") + info + std::string(" Assertion fail: ") + file_str_short + ":"
                                 + std::to_string(line) + " \n");
    }
}
...
#define CHECK_CUSPARSE(func)                                                                                           \
    {                                                                                                                  \
        ...
        if (status != CUSPARSE_STATUS_SUCCESS) {                                                                       \
            throw std::runtime_error(std::string("[ERROR] CUSPARSE API failed at line ")                           \
