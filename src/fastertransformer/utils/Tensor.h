...
struct Tensor {
    ...
    const float scale;
    const TensorFormat format;  // chw:0, c/32hw32:1
    ...
    Tensor(const MemoryType _where,
           ...
           const TensorFormat _format = TensorFormat::kLINEAR):
        where(_where), type(_type), shape(_shape), data(_data), scale(_scale), format(_format)
    {
    }
