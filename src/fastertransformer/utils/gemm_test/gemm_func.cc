...
size_t SVcalGemmTestBufSizeInByte(int batch_size,
                                  int seq_len,
                                  int head_num,
                                  int size_per_head,
                                  int inter_size,
                                  ...
                                  CublasDataType data_type)
{
    ...
    if (int8_mode > 0) {
        for (int i = 0; i < gemm_num; ++i) {
            ...
            size_t size = batchCount[i] * (m * k * sizeof(int8_t) + k * n * sizeof(int8_t) + m * n * sizeof(int));
            if (buf_size_in_byte < size) {
                buf_size_in_byte = size;
            }
        }
    }
    else {
        ...
        for (int i = 0; i < gemm_num; ++i) {
            int m = M[i], n = N[i], k = K[i];
            size_t size = batchCount[i] * (m * k + k * n + m * n);
            if (buf_size_in_byte < size) {
                buf_size_in_byte = size;
            }
        }
...
size_t GSDcalGemmTestBufSizeInByte(int batch_size,
                                  int seq_len,
                                  int head_num,
                                  int size_per_head,
                                  int inter_size,
                                  ...
                                  CublasDataType data_type)
{
    ...
    M[2] = seq_len;
    N[2] = seq_len;
    K[2] = size_per_head;// 240
    batchCount[2] = batch_size * head_num;

    M[3] = seq_len;
    N[3] = size_per_head;// 240
    K[3] = seq_len;
    batchCount[3] = batch_size * head_num;

    M[4] = batch_size * seq_len;
    N[4] = head_num * size_per_head;  // hidden_units_
    K[4] = N[4];
    batchCount[4] = 3;
    ...
    int batch_size, int seq_len, int head_num, int size_per_head, int inter_size, int hidden_units, int is_fp16)
{
    ...
    for (int i = 0; i < 10; ++i) {
        int m = M[i], n = N[i], k = K[i];
        size_t size = (m * n + m * k + n * k) * batchCount[i];
        if (size > max_size) {
            max_size = size;
        }
    }
