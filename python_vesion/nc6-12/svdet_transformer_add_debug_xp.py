...
class SVDetTransformerDecoder(TransformerLayerSequence):
    ...
    def forward_onnx(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if reg_branches is not None:
                tmp = reg_branches[lid](output)   ##tmp, New print , block_i.reg_branches.out.tmp
                
                assert reference_points.shape[-1] == 3   ##reference_points, New print, block_i.reg_branches.in.reference_points
...
class SVDetCrossAtten(BaseModule):
    ...
    def forward(self,
        ...
        ### NEED SAVE  attention_weights without bias !!!
        # t = attention_weights       ir.ca.attention_weights.out_nobias
        ###
        ...
        ### NEED SAVE
        # t = output      ca.output_permute
        ###  

        output = self.output_proj(output)
        ### NEED SAVE  output without bias !!!
        # t = output                 ca.out.subbias
        ###

        # (num_query, bs, embed_dims)
        pos_feat = self.position_encoder(inverse_sigmoid(reference_points_3d)).permute(1, 0, 2)
        ### NEED SAVE
        # t = pos_feat                  ca.out.pos_feat
        ###
        ### NEED SAVE
        # t = inp_residual + pos_feat       ir.ca.out.inp_res_pos_feat
        ###
...
def feature_sampling_onnx(mlvl_feats, reference_points, pc_range, img_shape, lidar2img):
    ...
    reference_points = torch.cat((reference_points, torch.ones_like(reference_points[..., :1])), -1)
    ### NEED SAVE
    # t = reference_points.permute(0, 2, 1).contiguous()                ir.ca.fs.rfpcat
    ###
    #   
    B, num_query = reference_points.size()[:2]
    num_cam = lidar2img.size(1)
    ...
    reference_points_cam = torch.matmul(lidar2img, reference_points).squeeze(-1) / img_shapes
    ### NEED SAVE
    # t = reference_points_cam.permute(0, 1, 3, 2).contiguous()            ca.fs.rfpcammatmul
    ###
    ...
    sampled_feats = []
    ### NEED SAVE
    # t = reference_points_cam.view(-1, num_query, 1, 2)              ca.fs.rfpcam
    ###  
    for lvl, feat in enumerate(mlvl_feats):
        B, N, C, H, W = feat.size()
        feat = feat.view(B*N, C, H, W)
...
# igs@igs:~/transformer$ conda activate bevnet
# (bevnet) igs@igs:~/transformer$ python3  ./svdet_transformer_export.py 