...
# base feature_sampling_onnx_v1.py
...
def feature_sampling_onnx(mlvl_feats, reference_points, pc_range, img_shape, lidar2img, pol_datas, cxy_cropxseyse_oxy):
    ...
    cxy_cropxseyse_oxy = cxy_cropxseyse_oxy.type_as(mlvl_feats[0])#[1, 4,8]
...
######################### begin
    ...
    reference_points_cam = reference_points_cam_x.view(1, num_cam, 4, num_query) #  [1, NC, 4, L] 
#################################### end as a kernel

    eps = 1e-5
    # [1, NC, 4, L]       auto l_idx  = threadIdx.x;  // map to L
                         #auto nc_idx = blockIdx.x;   // map to NC
    corners_flag = reference_points_cam[..., 2:3, :] > 0  # [1, NC, 1, L] 
    mask = (reference_points_cam[..., 2:3, :] > eps)      # [1, NC, 1, L] 

    reference_points_cam = reference_points_cam[..., 0:2, :] / torch.clamp(reference_points_cam[..., 2:3, :], min=0.01)
    print("reference_points_cam shape", reference_points_cam.shape)#[1, NC, 2, L]

    r = torch.norm(reference_points_cam[..., :2, :], dim=-2, keepdim=True) # based on row 
    print("r shape", r.shape) # [1, NC, 1, L]

    theta = torch.arctan(r)   # [1, NC, 1, L] means [1,4,1,L]
    ...
    print("--cdist shape", cdist.shape)                               # [1, 4, 1, 512]
    ...
    print("scale_x.shape", scale_x.shape) # [1, 4]
    print("crop_x_end.shape", crop_x_end.shape) # [1, 4]
    print("img_shape[0][1].shape", img_shape[0][1].shape) # []
    print("ori_inputs[:, :, 0].shape", ori_inputs[:, :, 0].shape) # [1, 4]
    print("crop_cfg_inputs[:, :, 0].shape", crop_cfg_inputs[:, :, 0].shape) # [1, 4]
    ...
    # savetensor_byrow(reference_points_cam.permute(0,1,3,2), 'reference_points_cam_v2.data', fmt = "%.6f")
...
#########################################
    ...
    print("mask.shape", mask.shape) # [1, 4, 1, 512]
    savetensor_byrow(mask.squeeze(), 'mask_v2.data', fmt = "%.6f")
