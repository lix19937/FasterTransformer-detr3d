...
class Model(nn.Module):
    ...
    def forward(self, value_0, value_1, value_2, lidar2img, img_shape):
...
def gen_sv_transformer_decoder():
    ...
    #value_3 = torch.randn(6, 256, 6, 23).cuda()    

    img_shape=torch.tensor([[192,736]])
    lidar2img=torch.randn(6,4,4).cuda()
    
    with torch.no_grad():
        _, _ = tfmodel(value_0,value_1,value_2,lidar2img,img_shape)
    
        torch.onnx.export(tfmodel,
                        (value_0,value_1,value_2, lidar2img, img_shape),
                        'sv_tf_decoder_e2e0907.onnx',
                        input_names=['value_0','value_1','value_2','lidar2img','img_shape'],
